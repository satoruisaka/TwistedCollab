"""
server.py - FastAPI REST API for TwistedCollab
...
class ChatMessageRequest(BaseModel):
    ...
    # Search mode: 'semantic' | 'keyword' | 'both'
    ...
    try:
        ...
        if settings.use_rag:
            ...
            if search_mode in ("semantic", "both"):
                retrieval_results = chat_manager.retrieval.semantic_search(
            ...
            if search_mode in ("keyword", "both"):
                kw_results = chat_manager.retrieval.keyword_search(
                    query=request.message,
                    scope=scope,
                    k=settings.top_k_retrieval
                )
                for r in kw_results:
                    snippet = _re.sub(r'<[^>]+>', '', r.get('snippet', ''))[:300]
                    context_items.append(ContextItem(
                        source=r.get('file_type', 'keyword'),
...
async def search(request: SearchRequest):
    ...
    try:
        ...
        # --- Semantic (FAISS) ---
        if search_mode in ("semantic", "both"):
            sem_results = retrieval_manager.semantic_search(
                query=request.query, scope=scope, k=request.k
            )
        ...
        # --- Keyword (FTS5) ---
        if search_mode in ("keyword", "both"):
            kw_results = retrieval_manager.keyword_search(
                query=request.query, scope=scope, k=request.k
            )
            for r in kw_results:
                combined.append({
                    "type": "keyword",
                    "chunk_id": None,
                    "text": _re.sub(r'<[^>]+>', '', r.get('snippet', '')),
                    "score": None,
                    "source": r.get('file_type', ''),
...
class IndexUpdateRequest(BaseModel):
    """Request for /api/update-faiss-index and /api/update-keyword-index"""
    sources: Optional[List[str]] = None   # None = all sources
    force: bool = False                   # force full rebuild


@app.post("/api/update-faiss-index")
async def update_faiss_index(request: IndexUpdateRequest):
    """
    Build / incrementally update FAISS indices.

    Runs FAISSIndexer.build_index() for each requested source.
    GPU-intensive – runs in a thread-pool executor to avoid blocking the
    event loop.
    """
    try:
        valid_sources = list(retrieval_manager.faiss.DATA_SOURCES.keys())
        ...
        if bad:
            raise HTTPException(
                status_code=400,
                detail=f"Unknown sources: {bad}. Valid: {valid_sources}"
            )

        logger.info(f"FAISS index update requested: {sources} (force={request.force})")
        ...
        for source in sources:
            stats = await loop.run_in_executor(
                None,
                lambda s=source: retrieval_manager.build_faiss_index(
                    s, force_rebuild=request.force
                )
            )
...
@app.post("/api/update-keyword-index")
async def update_keyword_index(request: IndexUpdateRequest):
    """
    Build / incrementally update the keyword (SQLite FTS5) index.

    Runs KeywordIndexer.index_source() for each requested source.
    CPU-bound – runs in a thread-pool executor.
    """
    try:
        valid_sources = list(retrieval_manager.keyword.DATA_SOURCES.keys())
        sources = request.sources or valid_sources
        ...
        logger.info(f"Keyword index update requested: {sources} (force={request.force})")
        ...
        for source in sources:
            summary = await loop.run_in_executor(
                None,
                lambda s=source: retrieval_manager.build_keyword_index(
                    s, force_reindex=request.force
                )
            )
        ...
        logger.error(f"Keyword index update error: {e}")
        raise HTTPException(status_code=500, detail=str(e))
